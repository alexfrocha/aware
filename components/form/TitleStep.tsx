...
export default function TitleStep({nextStep, title, setTitle}: {nextStep: () => void, title: string, setTitle: React.Dispatch<React.SetStateAction<string>>}) {
    ...
    return (
        <Step id="titlestep" className="z-[9]">        
            ...
            <Input value={title} onChange={(e: any) => setTitle(e.target.value)} id="titleinput" className={`w-[80%] duration-300 mb-2 md:w-[40%] md:text-[42px] border-b bg-none outline-none bg-black ${!title && missingTitle ? "border-b-red-500" : "border-b-zinc-300"}`} />
